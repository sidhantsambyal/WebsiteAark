...
export class Item {
  ...
  constructor(private _props: TProps) {
    ...
    const baseSize = 550;
    const baseSegments = 150;
    // reduce PNG display size to 50% (adjust multiplier as needed)
    const scale = 0.5;
    const scaledSize = baseSize * scale;
    const scaledSegments = Math.max(1, Math.floor(baseSegments * scale));
    // keep vertex density roughly constant by scaling segment count as well
    this._geometry = new PlaneGeometry(scaledSize, scaledSize, scaledSegments, scaledSegments);
